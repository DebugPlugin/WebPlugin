...
app.post("/api/extractor/import-har", extractorUpload.array("har", 10), async (req, res) => {
  ...
  for (const { name, data: har } of hars) {
    ...
    for (const entry of entries) {
      ...
      results.set(norm, {
        ...
        foundVia: "network:har (real)" + (redactedCount ? ` — ${redactedCount} dato(s) sensible(s) redactado(s)` : ""),
      ...
      while (idx < toFetch.length) {
        ...
        try {
          ...
          if (r.ok) {
            ...
            if (redactedCount) item.foundVia += ` — ${redactedCount} dato(s) sensible(s) redactado(s)`;
          } else {
  ...
  const pageUrl =
    sourceNames.length > 1
      ? `${sourceNames.length} archivos HAR importados (${sourceNames.join(", ")})`
  ...
  server.registerTool(
    "get_extracted_assets",
    {
      description:
        ...
        "read_extracted_asset for that. This is shared, not private to one person — it reflects whoever shared most recently.",
      inputSchema: { platform: platformParam },
    },
    async ({ platform }) => ({
