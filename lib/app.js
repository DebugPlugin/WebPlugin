...
app.post("/api/:platform/context", requirePlatform, async (req, res) => {
  try {
    const { apiCalls, screenshots, selectedVersion, includeCode, notes, extractedAssets } = req.body || {};
    await saveSharedContext(req.params.platform, { apiCalls, screenshots, selectedVersion, includeCode, notes, extractedAssets });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.get("/api/:platform/context", requirePlatform, async (req, res) => {
  try {
    ...
    res.json({
      ...
      apiCalls: Array.isArray(ctx?.apiCalls) ? ctx.apiCalls.map((c) => ({ title: c.title, method: c.method, url: c.url, status: c.status })) : [],
...
// before get_notes/get_screenshots/get_api_calls/get_extracted_assets, so Claude
...
async function doGetSharedContextOverview(platform) {
  ...
  const apiCallCount = Array.isArray(ctx.apiCalls) ? ctx.apiCalls.length : 0;
  const lines = [
    ...
    `API calls: ${apiCallCount ? `${apiCallCount} shared — call get_api_calls for the full responses` : "none shared"}`,
...
async function doGetApiCalls(platform) {
  const ctx = await loadSharedContext(platform);
  if (!ctx || !Array.isArray(ctx.apiCalls) || !ctx.apiCalls.length) {
    return "No API calls have been shared yet. Ask the user to use the \"Share with MCP\" button on the web page after making one or more API calls.";
  }
  const blocks = ctx.apiCalls.map(({ title, method, url, headers, status, json }, i) => {
    const headerLines = Object.entries(headers || {}).map(([k, v]) => `${k}: ${v}`).join("\n");
    return (
      `--- API call ${i + 1}/${ctx.apiCalls.length} — "${title || ""}" ---\n${method} ${url}\n${headerLines}\n\n` +
      `HTTP ${status}\n${json || ""}`
    );
  });
  return `[Shared ${ctx.savedAt}]\n\n` + blocks.join("\n\n");
}
...
function buildMcpServer() {
  const server = new McpServer({
    ...
    instructions:
      ...
      "actually available — and only then call the specific get_notes/get_screenshots/get_api_calls/get_extracted_assets/" +
  ...
  const CODE_ACCESS_DISABLED_MSG =
    ...
    "Don't attempt to read or search the code — answer using only what's already shared (API calls, screenshots, version) or ask the user to re-enable it.";
  ...
  server.registerTool(
    ...
    {
      description:
        "Check what's currently shared for a platform — version, API call count, screenshot count, whether notes exist, and how many " +
        "JS/CSS files were extracted — without fetching the actual content. Call this FIRST, before get_api_calls/get_screenshots/" +
  ...
  server.registerTool(
    ...
    {
      description:
        "Get all the real store API calls and responses shared from the web app for a given platform, via its \"Share with MCP\" button " +
        "(one entry per endpoint called before sharing, oldest first). " +
        "Check get_shared_context first — skip this call if it shows no API calls shared. " +
        "This is shared, not private to one person — it reflects whoever shared most recently.",
      inputSchema: { platform: platformParam },
    },
    async ({ platform }) => ({
      content: [{ type: "text", text: await doGetApiCalls(platform) }],
    })
  );
