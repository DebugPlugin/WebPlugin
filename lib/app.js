...
function chatSystemPrompt(platform) {
  ...
  return (
    ...
    "A user message may start with a '[Context: response from ...]' block containing the JSON response from a real store's API call — use it together with the plugin code to answer, e.g. to explain why a field is missing or malformed."
  );
}
  ...
  try {
    ...
    for (let i = 0; i < 8; i++) {
      const response = await anthropic.messages.create({
        model: "claude-opus-5",
        max_tokens: 4096,
        system: chatSystemPrompt(platform),
        tools: CHAT_TOOLS,
        messages: convo,
      });
