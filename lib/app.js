...
function doListVersions(platform) {
  return JSON.stringify(listLocalVersions(platform));
}
...
// ---------- Shared context (API calls / screenshots manually pushed from the browser) ----------
...
if (!fs.existsSync(SHARED_CONTEXT_DIR)) fs.mkdirSync(SHARED_CONTEXT_DIR, { recursive: true });

function sharedContextPath(platform) {
  return path.join(SHARED_CONTEXT_DIR, `${platform}.json`);
}

function saveSharedContext(platform, data) {
  fs.writeFileSync(sharedContextPath(platform), JSON.stringify({ ...data, savedAt: new Date().toISOString() }));
}

function loadSharedContext(platform) {
  try {
...
app.post("/api/:platform/context", requirePlatform, (req, res) => {
  try {
    const { apiCall, screenshots } = req.body || {};
    saveSharedContext(req.params.platform, { apiCall, screenshots });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
function doGetLastApiCall(platform) {
  const ctx = loadSharedContext(platform);
  if (!ctx || !ctx.apiCall) {
...
function doGetScreenshots(platform) {
  const ctx = loadSharedContext(platform);
...
function buildMcpServer() {
  ...
  server.registerTool(
    "list_versions",
    {
      description: "List plugin versions already downloaded locally and available to query, for a given platform.",
      inputSchema: { platform: platformParam },
    },
    async ({ platform }) => ({
      content: [{ type: "text", text: doListVersions(platform) }],
    })
  );
  ...
  server.registerTool(
    ...
    async ({ platform }) => ({
      content: [{ type: "text", text: doGetLastApiCall(platform) }],
    })
  );
  ...
  server.registerTool(
    ...
    async ({ platform }) => {
      const images = doGetScreenshots(platform);
      if (images.length === 0) {
