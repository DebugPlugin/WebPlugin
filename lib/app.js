...
function buildMcpServer() {
  ...
  server.registerTool(
    ...
    async ({ version }) => ({
      content: [{ type: "text", text: doListFiles(version) }],
    })
  );
  ...
  server.registerTool(
    ...
    async ({ version, path: relPath }) => ({
      content: [{ type: "text", text: doReadFile(version, relPath) }],
    })
  );
  ...
  server.registerTool(
    ...
    async ({ version, query }) => ({
      content: [{ type: "text", text: doSearchCode(version, query) }],
    })
  );
    ...
    for (let i = 0; i < 8; i++) {
      ...
      convo.push({
        ...
        content: toolUses.map((tu) => {
          try {
            return { type: "tool_result", tool_use_id: tu.id, content: runChatTool(tu.name, tu.input) };
          } catch (e) {
