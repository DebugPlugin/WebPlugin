...
app.post("/api/:platform/context", requirePlatform, async (req, res) => {
  try {
    const { apiCall, screenshots, selectedVersion, includeCode } = req.body || {};
    await saveSharedContext(req.params.platform, { apiCall, screenshots, selectedVersion, includeCode });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
function buildMcpServer() {
  const server = new McpServer({
    ...
    version: "1.0.0",
    instructions:
      "Only use the tools from this server when the user's message begins with the literal prefix \"DebugPlugin:\". " +
