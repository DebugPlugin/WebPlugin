...
// ---------- MCP server (pinned to the PrestaShop plugin) ----------

const MCP_PLATFORM = "prestashop";
...
function buildMcpServer() {
  ...
  server.registerTool(
    ...
    async () => ({
      content: [{ type: "text", text: doListVersions(MCP_PLATFORM) }],
    })
  );
  ...
  server.registerTool(
    ...
    async ({ version }) => ({
      content: [{ type: "text", text: await doListFiles(MCP_PLATFORM, version) }],
    })
  );
  ...
  server.registerTool(
    ...
    async ({ version, path: relPath }) => ({
      content: [{ type: "text", text: await doReadFile(MCP_PLATFORM, version, relPath) }],
    })
  );
  ...
  server.registerTool(
    ...
    async ({ version, query }) => ({
      content: [{ type: "text", text: await doSearchCode(MCP_PLATFORM, version, query) }],
    })
  );
