...
app.post("/api/:platform/context", requirePlatform, (req, res) => {
  try {
    ...
    saveSharedContext(req.params.platform, { apiCall, screenshots, selectedVersion });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
function doGetSelectedVersion(platform) {
  const ctx = loadSharedContext(platform);
...
function buildMcpServer() {
  ...
  server.registerTool(
    ...
    async ({ platform }) => ({
      content: [{ type: "text", text: doGetSelectedVersion(platform) }],
    })
  );
